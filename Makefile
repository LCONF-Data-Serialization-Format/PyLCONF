...
.PHONY: help clean cleanall clean_force_exclude_files tests tests_cover build_ext build_ext_force build_force install install_develop uninstall_develop dist pypi_all check_setup cython_annotate code_analysis_pylint
...
help:
	...
	@echo '  pypi_all                   force re-compile and re-register/upload to pypi'
...
tests_cover:
	...
	${PYTHON} setup.py build_ext --inplace
	${PYTHON} setup.py clean
	${PYTHON} setup.py nosetests --cover-branches --with-coverage --cover-html --cover-erase --cover-package=PyLCONF
	${PYTHON} setup.py clean
